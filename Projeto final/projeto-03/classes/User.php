...
class User{
    ...
    public function dbConnect($conString, $user, $pass){
        if(session_status() === PHP_SESSION_ACTIVE){
            try {
                ...
                $this->msg = 'A conexão não ocorreu!';
                return false;
            }
        }else{
            $this->msg = 'Sessão não iniciou.';
            return false;
        }
    }
        ...
        if(is_null($this->pdo)){
            ...
            if(password_verify($password,$user['password'])){
                if($user['wrong_logins'] <= $this->permitedAttemps){
                    ...
                    $this->msg = 'Esta conta esta bloqueada, entre em contato com o suporte.';
                    return false;
                }
            }else{
                $this->registerWrongLoginAttemp($email);
                $this->msg = 'Dados inválidos ou conta não ativada.';
                return false;
            } 
        }
    }
    ...
    public function registration($email,$fname,$lname,$pass){
        ...
        if($stmt->execute([$fname,$lname,$email,$pass,$confCode])){
            if($this->sendConfirmationEmail($email)){
                ...
                $this->msg = 'Envio do e-mail de confirmação falhou.';
                return false; 
            }
        }else{
            $this->msg = 'Insira um novo usuário.';
            return false;
        }
    }
    ...
    */
    private function sendConfirmationEmail($email){
        ...
        $message = 'COnfirme seu cadastro utilizando este código de confirmação: '.$code['confirm_code'];
    ...
    public function emailActivation($email,$confCode){
        ...
        if($stmt->rowCount()>0){
            ...
            if(!empty($user['email'])){
            	...
            	$this->msg = 'A ativação da conta falhou.';
            	return false;
            }            
        }else{
            $this->msg = 'A ativação da conta falhou.';
    ...
    public function passwordChange($id,$pass){
        ...
        if(isset($id) && isset($pass)){
            ...
            if($stmt->execute([$id,$this->hashPass($pass)])){
                ...
                $this->msg = 'Alteração de senha falhou.';
                return false;
            }
        }else{
            $this->msg = 'Forneça um ID e uma senha.';
            return false;
        }
    }
    ...
    public function assignRole($id,$role){
        ...
        if(isset($id) && isset($role)){
            ...
            if($stmt->execute([$id,$role])){
                ...
                $this->msg = 'A definição de grupo falhou.';
                return false;
            }
        }else{
            $this->msg = 'Forneça um grupo para este usuário.';
            return false;
        }
    }
    ...
    public function userUpdate($id,$fname,$lname){
        ...
        if(isset($id) && isset($fname) && isset($lname)){
            ...
            if($stmt->execute([$id,$fname,$lname])){
                ...
                $this->msg = 'Falha ao atualizar os dados do usuário.';
                return false;
            }
        }else{
            $this->msg = 'Forneça os dados corretos para o usuário.';
            return false;
        }
    }
